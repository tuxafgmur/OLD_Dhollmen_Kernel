...
void erandom_get_random_bytes(char *buf, size_t count)
{
	...
	*/
	...
	/* We seed erandom as late as possible, hoping that the kernel's main RNG
	 * is already restored in the boot sequence (not critical, but better)
	 */
...
static int frandom_init_module(void)
{
	...
	*/       
	if (frandom_bufsize < 256) {
		printk(KERN_ERR "frandom: Refused to load with frandom_bufsize=%d < 256\n",frandom_bufsize);
		return -EINVAL;
	}
	if ((frandom_chunklimit != 0) && (frandom_chunklimit < 256)) {
		printk(KERN_ERR "frandom: Refused to load with frandom_chunklimit=%d < 256 and != 0\n",frandom_chunklimit);
		return -EINVAL;
	}
	...
	/* This specific buffer is only used for seeding, so we need 256 bytes exactly */
